...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isExist) return failure('Business profile already exist.', 409);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    //Verify if phone number was updated also
    const phone_number_combined = `(${data.phone_country_code})${data.phone_number}`;
    ...
    const updatedBusinessProfile = await prisma.businessProfile.update({
      where: { id: businessProfileId },
      data: {
        company_name: data.company_name ? data.company_name : profile.company_name,
        phone_number: data.phone_country_code || data.phone_number ? phone_number_combined : profile.phone_number,
        company_location: data.company_location ? data.company_location : profile.company_location,
        company_website: data.company_website ? data.company_website : profile.company_website,
        business_industry: data.business_industry ? data.business_industry : profile.business_industry,
        business_category: data.business_category ? data.business_category : profile.business_category,
        annual_revenue: data.annual_revenue ? data.annual_revenue : profile.annual_revenue,
        updated_at: new Date(),
      },
    });
